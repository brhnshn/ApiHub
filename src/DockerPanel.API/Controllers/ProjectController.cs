...
public class ProjectController : ControllerBase
{
    ...
    public async Task<IActionResult> GetAll()
    {
        ...
        // Aktif çalışma zamanı otomatik iyileştirici (stuck in Provisioning > 10 saniye veya zaman dilimi uyumsuzluğu)
        bool changed = false;
        foreach (var project in projects)
        {
            if (project.Type == ProjectType.NativeProject)
            {
                ...
                {
                    SystemLogQueue.Log("warning", $"[Self-Healing] {project.Name} projects.conf onarımı yapılamadı: {ex.Message}");
                }
            }

            // Sync actual runtime status to database
            bool isActuallyRunning = false;
            try
            {
                if (project.Type == ProjectType.DockerContainer && !string.IsNullOrEmpty(project.DockerContainerId))
                {
                    isActuallyRunning = await _containerService.IsContainerRunningAsync(project.DockerContainerId);
                }
                else if (project.Type == ProjectType.NativeProject)
                {
                    isActuallyRunning = await _processManagerService.IsProcessRunningAsync(project.Name);
                }
            ...
            {
                SystemLogQueue.Log("warning", $"[Status-Sync] {project.Name} durum kontrolü sırasında hata: {ex.Message}");
            }

            if (isActuallyRunning && project.Status != ProjectStatus.Running && project.Status != ProjectStatus.Provisioning)
            {
                MarkRunning(project);
                _dbContext.Entry(project).State = EntityState.Modified;
                changed = true;
                SystemLogQueue.Log("info", $"[Status-Sync] {project.Name} dışarıdan başlatılmış olarak algılandı, durum 'Running' olarak güncellendi.");
            }
            else if (!isActuallyRunning && project.Status == ProjectStatus.Running)
            {
                MarkStopped(project);
                _dbContext.Entry(project).State = EntityState.Modified;
                changed = true;
                SystemLogQueue.Log("info", $"[Status-Sync] {project.Name} dışarıdan durdurulmuş olarak algılandı, durum 'Stopped' olarak güncellendi.");
            }
            else
            {
                // Standart provisioning onarıcı kodunu koruyalım
                var age = DateTimeOffset.UtcNow - project.CreatedAt;
                var absDiffMinutes = Math.Abs(age.TotalMinutes);

                if (project.Status == ProjectStatus.Provisioning && (age.TotalSeconds > 10 || absDiffMinutes > 2))
                {
                    ...
                    _dbContext.Entry(project).State = EntityState.Modified;
                    changed = true;
                    SystemLogQueue.Log("warning", $"[Self-Healing] {project.Name} ({project.Type}) stuck in Provisioning (Age: {age.TotalSeconds:0}s). Status healed to {project.Status}.");
                }
            }
        }
        if (changed)
